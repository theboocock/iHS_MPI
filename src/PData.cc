...
int main(int argc, char **argv){
    //ID for the master process in IHS calculation.
    int master = 0;
    int masters = 1;
    PData pd;
    int taskid, num_tasks;
    
    //Processor id of the process that needs a snp
    int get_snp_proc_id;
    // Improvement for parralelisation could be made here. Using 
    // pthread / mpi
    ...
    pd.load_info(argv[1]);
    pd.load_data(argv[2]);
    // Send the current snp to the  
    int current_snp;   
    int recieved_id; 
    int flag;
    for(int i=0;i<pd.snp_num;i++){
       //You are the master process 
       if(taskid == 0){
            //printf("MASTER \n");
            //fflush(stdout); 
            MPI_Recv(&recieved_id,1,MPI_INT,MPI_ANY_SOURCE,0,MPI_COMM_WORLD,MPI_STATUS_IGNORE);
            MPI_Send(&i,1,MPI_INT,recieved_id,0,MPI_COMM_WORLD);
            //printf("MASTER out \n");
            //fflush(stdout); 
        }else{
            //printf("SLAVE \n");
            //fflush(stdout); 
            MPI_Send(&taskid,1,MPI_INT,master,0,MPI_COMM_WORLD);
            MPI_Recv(&current_snp,1,MPI_INT,master,0,MPI_COMM_WORLD,MPI_STATUS_IGNORE);
            pd.compute_data(current_snp);
            if(current_snp == -1){
                break;
            }
            pd.compute_data(current_snp);
        ...
        // clean up all the processes enusuring they exit correctly    
    int exit_snp_code = -1;
    int num_finished = 0;
    if(num_tasks > 2){
        while(num_finished++ < (num_tasks - masters)){
            if(master == taskid){ 
            MPI_Recv(&recieved_id,1,MPI_INT,MPI_ANY_SOURCE,0,MPI_COMM_WORLD,MPI_STATUS_IGNORE);
            MPI_Send(&exit_snp_code,1,MPI_INT,recieved_id,0,MPI_COMM_WORLD);
            }
        }
    }
    ...
    printf("Process %d of %d is finished\n",taskid,num_tasks);
    MPI_Finalize();
